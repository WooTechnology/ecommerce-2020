...
const Card = ({
	...
	const showaddToCartButton = (addToCartButton) => {
		return product.stock ?  (
			(addToCartButton || !product.quantity) && (
				<button className="btn btn-success btn-block rounded" onClick={addToCart}>
					Add to Cart
				</button>
			) 
		): (
			<div className="text-danger"><b>Out of stock!!</b></div>
		);
	};
	...
	const showgoToCartButton = (goToCartButton) => {
		return (
			(goToCartButton || product.quantity) && (
				<Link to="/cart" className="btn btn-secondary btn-block rounded">Go to Cart</Link>
			)
		);
	};
	...
	return (
		<div className="card text-secondary m-2" style={{ width: "16rem" }}>
			...
			<div className="card-body">
				...
				{showgoToCartButton(goToCartButton)}
